const params = {
  ...
  content: [
    ...
    { type: "line", text: `чтобы исходные данные были заданы в конкретном алфавите и могли принимать значения из некоторого множества, т.е. носили массовый характер;` },
    { type: "line", text: `чтобы процесс переработки данных состоял из отдельных дискретных шагов и был детерминированным;` },
    { type: "line", text: `чтобы были четко указаны условия остановки процесса, и что следует считать результатом процесса.` },
    ...
    переходит в новое состояние q+ (или остается в старом) и передвигается: вправо (П), влево (Л) или остается в прежнем положении (Н).` },
    ...
    { type: "parag", text: `Работу машины Тьюринга можно описать как последовательную смену ее конфигураций, причем машина переходит от конфигурации Kt к конфигурации Kt+1 в соответствии со своей программой.
    Любая начальная конфигурация K0, которой соответствует состояние q1, порождает последовательность конфигураций K0,K1,K2, …,Kt,… .` },
    ...
    { type: "parag", text: `Если последовательность конфигураций K0,K1,K2, …,Kt,… никогда не обрывается, т.е. машина работает вечно (“зацикливается”), будем говорить, что машина Тьюринга неприменима к конфигурации K0.` },
    ...
    причем головка в начальном состоянии q1 Q обозревает самый левый символ словаX, т.е. начальная конфигурация имеет вид q1X. Результирующая конфигурация имеет вид q0 f(X).` },
    { type: "parag", text: `В этом случае будем говорить, что машина Тьюринга вычисляет словарную функцию f, причем слово f(X) есть значение этой функции для аргумента X. Числовые функции – это частный случай словарных,
    ...
    { type: "parag", text: `В теории алгоритмов рассматриваются примеры специализированных машин Тьюринга с ленточным алфавитом A={Λ,+,1}, алфавитом состояний {q0,q1,q2, …,qn} и алфавитом перемещенийD {П,Л,Н}.
